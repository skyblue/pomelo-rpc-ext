...
pro.connect = function(tracer, cb) {
  ...
  var self = this;
  this.socket = client.connect(this.host + ':' + this.port, {'force new connection': true, 'reconnect': true});
...
pro.send = function(tracer, msg, opts, cb) {
  tracer.info('client', __filename, 'send', 'sio-mailbox try to send');
  if(!this.connected) {
    tracer.error('client', __filename, 'send', 'sio-mailbox not init');
    utils.invokeCallback(cb, tracer, new Error('sio-mailbox is not init'));
    return;
  }

  if(this.closed) {
    tracer.error('client', __filename, 'send', 'mailbox has already closed');
    utils.invokeCallback(cb, tracer, new Error('sio-mailbox has already closed'));
    return;
  }
